...
import { HistoricalSiteRank, UserRole } from '../generated/prisma';
import { PrismaService } from '../src/database/prisma.service';
import { createTestApp } from './utils/create-test-app';

describe('Historical sites (e2e)', () => {
  let app: INestApplication<App>;
  let prisma: PrismaService;
  const marker = randomUUID();
  const prefix = `hsite-${marker}`;
  ...
  beforeEach(async () => {
    await prisma.historicalSite.deleteMany({
      where: { id: { contains: marker } },
    });
    await prisma.historicalSite.createMany({
      data: [
        {
          id: ids.first,
          name: 'Di Tích Thứ Nhất',
        ...
        {
          id: ids.second,
          name: 'Di Tích Thứ Hai',
          rank: HistoricalSiteRank.CAP_QUOC_GIA,
          address: 'Địa chỉ B',
          recognizedYear: 2015,
          summary: 'Tóm tắt B',
          history: ['Lịch sử B'],
  ...
  it('public list excludes hidden records, sorts by sortOrder and filters rank', async () => {
    const response = await request(app.getHttpServer())
      .get('/api/v1/historical-sites')
      .expect(200);
  ...
  it('public detail hides existence of inactive and missing records', async () => {
    for (const id of [ids.hidden, `${prefix}-missing`]) {
      const response = await request(app.getHttpServer())
        .get(`/api/v1/historical-sites/${id}`)
        .expect(404);
      expect(response.body.error.code).toBe('HISTORICAL_SITE_NOT_FOUND');
    }
  });

  it('protects all admin endpoints by authentication and role', async () => {
    await request(app.getHttpServer())
      .get('/api/v1/admin/historical-sites')
      .expect(401);
  ...
  it('generates a stable Vietnamese slug and rejects a duplicate slug', async () => {
    ...
    const created = await request(app.getHttpServer())
      .post('/api/v1/admin/historical-sites')
      .set('Authorization', `Bearer ${admin.token}`)
      .send(body({ name: `Di Tích Mới ${marker}` }))
      .expect(201);
    ...
    const duplicate = await request(app.getHttpServer())
      ...
      .expect(409);
    expect(duplicate.body.error.code).toBe('HISTORICAL_SITE_SLUG_EXISTS');
  });

  it('rejects an out-of-range recognizedYear on create and update', async () => {
    const admin = await login('year-admin', UserRole.ADMIN);
    await request(app.getHttpServer())
      .post('/api/v1/admin/historical-sites')
      .set('Authorization', `Bearer ${admin.token}`)
      .send(body({ id: `${prefix}-year-past`, recognizedYear: 1899 }))
      .expect(400);
    ...
    await request(app.getHttpServer())
      ...
      .send(
        body({
          id: `${prefix}-year-future`,
          recognizedYear: new Date().getFullYear() + 1,
        }),
      )
      .expect(400);

    await request(app.getHttpServer())
      .put(`/api/v1/admin/historical-sites/${ids.first}`)
      .set('Authorization', `Bearer ${admin.token}`)
      .send(body({ version: 1, recognizedYear: -5 }))
      .expect(400);
  });
  ...
  it('updates only the target and returns latest data on a stale version conflict', async () => {
    ...
    const otherBefore = await prisma.historicalSite.findUniqueOrThrow({
      where: { id: ids.second },
    });
    ...
    await request(app.getHttpServer())
      ...
      .send(body({ version: 1, name: 'Di Tích Đã Cập Nhật' }))
      .expect(200);
    ...
    const conflict = await request(app.getHttpServer())
      ...
      .expect(409);
    expect(conflict.body.error.code).toBe('HISTORICAL_SITE_VERSION_CONFLICT');
    expect(conflict.body.error.details[0].latest.name).toBe(
      'Di Tích Đã Cập Nhật',
    );

    const otherAfter = await prisma.historicalSite.findUniqueOrThrow({
      where: { id: ids.second },
    });
  ...
  it('reorders atomically and rolls back when any id is invalid', async () => {
    ...
    await request(app.getHttpServer())
      .patch('/api/v1/admin/historical-sites/reorder')
      .set('Authorization', `Bearer ${admin.token}`)
      .send({
        items: [
    ...
    expect(
      (
        await prisma.historicalSite.findUniqueOrThrow({
          where: { id: ids.first },
        })
      ).sortOrder,
    ).toBe(0);
  ...
  it('status changes public visibility and delete removes the record', async () => {
    ...
    await request(app.getHttpServer())
      .patch(`/api/v1/admin/historical-sites/${ids.first}/status`)
      .set('Authorization', `Bearer ${admin.token}`)
      .send({ isActive: false, version: 1 })
      .expect(200);
    await request(app.getHttpServer())
      .get(`/api/v1/historical-sites/${ids.first}`)
      .expect(404);

    await request(app.getHttpServer())
      .delete(`/api/v1/admin/historical-sites/${ids.second}`)
      .set('Authorization', `Bearer ${admin.token}`)
      .expect(204);
    expect(
      await prisma.historicalSite.findUnique({ where: { id: ids.second } }),
    ).toBeNull();
  });
  ...
  it('rejects missing media and non-image media', async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .expect(400);
    expect(response.body.error.code).toBe('INVALID_HISTORICAL_SITE_MEDIA');
  });
});
