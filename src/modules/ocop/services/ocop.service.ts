import { HttpStatus, Injectable } from '@nestjs/common';
import {
  MediaResourceType,
  Prisma,
  type Media,
  type OcopProduct,
  type User,
} from '../../../../generated/prisma';
import { ErrorCode } from '../../../common/constants/error-codes.constant';
import { paginate } from '../../../common/pagination/pagination.util';
import { AppException } from '../../../common/exceptions/app.exception';
import { slugify } from '../../../common/utils/slugify.util';
import { PrismaService } from '../../../database/prisma.service';
import { MediaService } from '../../media/services/media.service';
import {
  ReorderOcopProductsDto,
  UpdateOcopProductStatusDto,
} from '../dto/ocop-action.dto';
import { QueryOcopAdminDto, QueryOcopPublicDto } from '../dto/query-ocop.dto';
import {
  OcopAdminResponseDto,
  OcopListItemResponseDto,
  OcopResponseDto,
} from '../dto/ocop-response.dto';
import {
  CreateOcopProductDto,
  UpdateOcopProductDto,
} from '../dto/upsert-ocop.dto';
import {
  OCOP_CATEGORY_FROM_DB,
  OCOP_CATEGORY_TO_DB,
  OCOP_RATING_FROM_DB,
  OCOP_RATING_TO_DB,
} from '../ocop.constants';

type OcopProductRecord = OcopProduct & {
  media: Media | null;
  createdBy: Pick<User, 'displayName'> | null;
  updatedBy: Pick<User, 'displayName'> | null;
};

const includeOcopProduct = {
  media: true,
  createdBy: { select: { displayName: true } },
  updatedBy: { select: { displayName: true } },
};

@Injectable()
export class OcopService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly mediaService: MediaService,
  ) {}

  async publicList(
    query: QueryOcopPublicDto,
  ): Promise<OcopListItemResponseDto[]> {
    const records = await this.prisma.ocopProduct.findMany({
      where: {
        isActive: true,
        ...(query.category
          ? { category: OCOP_CATEGORY_TO_DB[query.category] }
          : {}),
        ...(query.rating ? { rating: OCOP_RATING_TO_DB[query.rating] } : {}),
      },
      orderBy: [{ sortOrder: 'asc' }, { createdAt: 'asc' }, { id: 'asc' }],
      include: includeOcopProduct,
    });
    return records.map((record) => this.toListDto(record));
  }

  async publicDetail(id: string): Promise<OcopResponseDto> {
    const record = await this.prisma.ocopProduct.findFirst({
      where: { id, isActive: true },
      include: includeOcopProduct,
    });
    if (!record) throw this.notFound();
    return this.toPublicDto(record);
  }

  async adminList(query: QueryOcopAdminDto) {
    const where: Prisma.OcopProductWhereInput = {
      ...(query.category
        ? { category: OCOP_CATEGORY_TO_DB[query.category] }
        : {}),
      ...(query.rating ? { rating: OCOP_RATING_TO_DB[query.rating] } : {}),
      ...(query.search
        ? {
            OR: [
              { name: { contains: query.search, mode: 'insensitive' } },
              { producer: { contains: query.search, mode: 'insensitive' } },
            ],
          }
        : {}),
    };
    const [records, total] = await this.prisma.$transaction([
      this.prisma.ocopProduct.findMany({
        where,
        skip: (query.page - 1) * query.limit,
        take: query.limit,
        orderBy: [{ sortOrder: 'asc' }, { createdAt: 'asc' }, { id: 'asc' }],
        include: includeOcopProduct,
      }),
      this.prisma.ocopProduct.count({ where }),
    ]);
    return paginate(
      records.map((record) => this.toAdminDto(record)),
      query.page,
      query.limit,
      total,
    );
  }

  async adminDetail(id: string): Promise<OcopAdminResponseDto> {
    return this.toAdminDto(await this.findOrThrow(id));
  }

  async create(
    actorId: string,
    dto: CreateOcopProductDto,
  ): Promise<OcopAdminResponseDto> {
    const id = dto.id ?? slugify(dto.name);
    const duplicate = await this.prisma.ocopProduct.findUnique({
      where: { id },
      select: { id: true },
    });
    if (duplicate) throw this.slugExists();
    await this.validateMedia(dto.mediaId, actorId, id);

    const record = await this.prisma.$transaction(async (tx) => {
      const sortOrder = dto.sortOrder ?? (await this.nextSortOrder(tx));
      const created = await tx.ocopProduct.create({
        data: {
          id,
          name: dto.name,
          category: OCOP_CATEGORY_TO_DB[dto.category],
          rating: OCOP_RATING_TO_DB[dto.rating],
          producer: dto.producer,
          address: dto.address,
          priceRange: dto.priceRange,
          summary: dto.summary,
          description: dto.description,
          highlights: dto.highlights,
          contactNote: dto.contactNote,
          contactPhone: dto.contactPhone,
          mediaId: dto.mediaId ?? null,
          imageAlt: dto.imageAlt,
          sortOrder,
          isActive: dto.isActive ?? true,
          createdById: actorId,
          updatedById: actorId,
        },
        include: includeOcopProduct,
      });
      await this.claimMedia(tx, dto.mediaId, actorId, id);
      await tx.auditLog.create({
        data: {
          actorId,
          action: 'OCOP_PRODUCT_CREATED',
          resourceType: 'OcopProduct',
          resourceId: id,
          newData: { version: 1 },
        },
      });
      return created;
    });
    return this.toAdminDto(record);
  }

  async update(
    actorId: string,
    id: string,
    dto: UpdateOcopProductDto,
  ): Promise<OcopAdminResponseDto> {
    const current = await this.findOrThrow(id);
    await this.validateMedia(dto.mediaId, actorId, id);

    const { record, deletedMediaId } = await this.prisma.$transaction(
      async (tx) => {
        const updated = await tx.ocopProduct.updateMany({
          where: { id, version: dto.version },
          data: {
            name: dto.name,
            category: OCOP_CATEGORY_TO_DB[dto.category],
            rating: OCOP_RATING_TO_DB[dto.rating],
            producer: dto.producer,
            address: dto.address,
            priceRange: dto.priceRange,
            summary: dto.summary,
            description: dto.description,
            highlights: dto.highlights,
            contactNote: dto.contactNote,
            contactPhone: dto.contactPhone,
            mediaId: dto.mediaId ?? null,
            imageAlt: dto.imageAlt,
            ...(dto.sortOrder === undefined
              ? {}
              : { sortOrder: dto.sortOrder }),
            ...(dto.isActive === undefined ? {} : { isActive: dto.isActive }),
            version: { increment: 1 },
            updatedById: actorId,
          },
        });
        if (updated.count !== 1) throw await this.versionConflict(tx, id);

        const deletedMediaId = await this.mediaService.releaseSingularMedia(
          tx,
          {
            previousMediaId: current.mediaId,
            nextMediaId: dto.mediaId ?? null,
            resourceType: MediaResourceType.OCOP,
            resourceId: id,
          },
        );
        await this.claimMedia(tx, dto.mediaId, actorId, id);
        await tx.auditLog.create({
          data: {
            actorId,
            action: 'OCOP_PRODUCT_UPDATED',
            resourceType: 'OcopProduct',
            resourceId: id,
            oldData: { version: dto.version },
            newData: { version: dto.version + 1 },
          },
        });
        const record = await tx.ocopProduct.findUniqueOrThrow({
          where: { id },
          include: includeOcopProduct,
        });
        return { record, deletedMediaId };
      },
    );
    await this.mediaService.purgeStorage([deletedMediaId]);
    return this.toAdminDto(record);
  }

  async updateStatus(
    actorId: string,
    id: string,
    dto: UpdateOcopProductStatusDto,
  ): Promise<OcopAdminResponseDto> {
    if (dto.isActive) {
      const current = await this.findOrThrow(id);
      this.assertContactPhoneForActive(current.contactPhone);
    }
    const record = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.ocopProduct.updateMany({
        where: { id, version: dto.version },
        data: {
          isActive: dto.isActive,
          version: { increment: 1 },
          updatedById: actorId,
        },
      });
      if (updated.count !== 1) throw await this.versionConflict(tx, id);
      await tx.auditLog.create({
        data: {
          actorId,
          action: dto.isActive ? 'OCOP_PRODUCT_SHOWN' : 'OCOP_PRODUCT_HIDDEN',
          resourceType: 'OcopProduct',
          resourceId: id,
          newData: { isActive: dto.isActive },
        },
      });
      return tx.ocopProduct.findUniqueOrThrow({
        where: { id },
        include: includeOcopProduct,
      });
    });
    return this.toAdminDto(record);
  }

  async reorder(
    actorId: string,
    dto: ReorderOcopProductsDto,
  ): Promise<OcopAdminResponseDto[]> {
    const ids = dto.items.map((item) => item.id);
    if (new Set(ids).size !== ids.length) {
      throw new AppException(
        ErrorCode.VALIDATION_ERROR,
        'Danh sách sắp xếp chứa ID trùng',
      );
    }
    return this.prisma.$transaction(async (tx) => {
      const found = await tx.ocopProduct.count({
        where: { id: { in: ids } },
      });
      if (found !== ids.length) throw this.notFound();
      for (const item of dto.items) {
        await tx.ocopProduct.update({
          where: { id: item.id },
          data: {
            sortOrder: item.sortOrder,
            version: { increment: 1 },
            updatedById: actorId,
          },
        });
      }
      await tx.auditLog.create({
        data: {
          actorId,
          action: 'OCOP_PRODUCTS_REORDERED',
          resourceType: 'OcopProduct',
          newData: {
            items: dto.items.map(({ id, sortOrder }) => ({ id, sortOrder })),
          },
        },
      });
      const records = await tx.ocopProduct.findMany({
        where: { id: { in: ids } },
        orderBy: [{ sortOrder: 'asc' }, { id: 'asc' }],
        include: includeOcopProduct,
      });
      return records.map((record) => this.toAdminDto(record));
    });
  }

  async remove(actorId: string, id: string): Promise<void> {
    const current = await this.findOrThrow(id);
    const deletedMediaId = await this.prisma.$transaction(async (tx) => {
      await tx.ocopProduct.delete({ where: { id } });
      const deletedMediaId = await this.mediaService.releaseSingularMedia(tx, {
        previousMediaId: current.mediaId,
        nextMediaId: null,
        resourceType: MediaResourceType.OCOP,
        resourceId: id,
      });
      await tx.auditLog.create({
        data: {
          actorId,
          action: 'OCOP_PRODUCT_DELETED',
          resourceType: 'OcopProduct',
          resourceId: id,
        },
      });
      return deletedMediaId;
    });
    await this.mediaService.purgeStorage([deletedMediaId]);
  }

  private async findOrThrow(id: string): Promise<OcopProductRecord> {
    const record = await this.prisma.ocopProduct.findUnique({
      where: { id },
      include: includeOcopProduct,
    });
    if (!record) throw this.notFound();
    return record;
  }

  private toListDto(record: OcopProductRecord): OcopListItemResponseDto {
    return {
      id: record.id,
      name: record.name,
      category: OCOP_CATEGORY_FROM_DB[record.category],
      rating: OCOP_RATING_FROM_DB[record.rating],
      producer: record.producer,
      address: record.address,
      priceRange: record.priceRange,
      summary: record.summary,
      imageUrl: record.media?.originalUrl ?? null,
      imageAlt: record.imageAlt,
      sortOrder: record.sortOrder,
      updatedAt: record.updatedAt,
    };
  }

  private toPublicDto(record: OcopProductRecord): OcopResponseDto {
    return {
      ...this.toListDto(record),
      description: this.readStringArray(record.description),
      highlights: this.readStringArray(record.highlights),
      contactNote: record.contactNote,
      contactPhone: record.contactPhone,
    };
  }

  private toAdminDto(record: OcopProductRecord): OcopAdminResponseDto {
    return {
      ...this.toPublicDto(record),
      mediaId: record.mediaId,
      isActive: record.isActive,
      version: record.version,
      createdAt: record.createdAt,
      createdBy: record.createdBy?.displayName ?? null,
      updatedBy: record.updatedBy?.displayName ?? null,
    };
  }

  private readStringArray(value: Prisma.JsonValue): string[] {
    return Array.isArray(value)
      ? value.filter((item): item is string => typeof item === 'string')
      : [];
  }

  private assertContactPhoneForActive(contactPhone: string | null): void {
    if (!contactPhone || contactPhone.trim().length === 0) {
      throw new AppException(
        ErrorCode.VALIDATION_ERROR,
        'Cần có số điện thoại liên hệ trước khi hiển thị sản phẩm này',
      );
    }
  }

  private async validateMedia(
    mediaId: string | null | undefined,
    actorId: string,
    ocopProductId: string,
  ): Promise<void> {
    if (!mediaId) return;
    const media = await this.prisma.media.findFirst({
      where: { id: mediaId, deletedAt: null },
    });
    const validImage = media?.mimeType.startsWith('image/');
    const usable =
      media &&
      ((media.resourceType === null && media.ownerId === actorId) ||
        (media.resourceType === MediaResourceType.OCOP &&
          (media.resourceId === ocopProductId || media.resourceId === null)));
    if (!validImage || !usable) {
      throw new AppException(
        ErrorCode.INVALID_OCOP_PRODUCT_MEDIA,
        'Ảnh không tồn tại hoặc không được phép sử dụng',
        HttpStatus.BAD_REQUEST,
      );
    }
  }

  private async claimMedia(
    tx: Prisma.TransactionClient,
    mediaId: string | null | undefined,
    actorId: string,
    ocopProductId: string,
  ): Promise<void> {
    if (!mediaId) return;
    const claimed = await tx.media.updateMany({
      where: {
        id: mediaId,
        ownerId: actorId,
        resourceType: null,
        deletedAt: null,
      },
      data: {
        resourceType: MediaResourceType.OCOP,
        resourceId: ocopProductId,
      },
    });
    if (claimed.count === 0) {
      const existing = await tx.media.findFirst({
        where: {
          id: mediaId,
          resourceType: MediaResourceType.OCOP,
          OR: [{ resourceId: ocopProductId }, { resourceId: null }],
          deletedAt: null,
        },
      });
      if (!existing)
        throw new AppException(
          ErrorCode.INVALID_OCOP_PRODUCT_MEDIA,
          'Ảnh đã được nội dung khác sử dụng',
          HttpStatus.CONFLICT,
        );
    }
  }

  private async nextSortOrder(tx: Prisma.TransactionClient): Promise<number> {
    const result = await tx.ocopProduct.aggregate({
      _max: { sortOrder: true },
    });
    return (result._max.sortOrder ?? -1) + 1;
  }

  private async versionConflict(
    tx: Prisma.TransactionClient,
    id: string,
  ): Promise<AppException> {
    const latest = await tx.ocopProduct.findUnique({
      where: { id },
      include: includeOcopProduct,
    });
    if (!latest) return this.notFound();
    return new AppException(
      ErrorCode.OCOP_PRODUCT_VERSION_CONFLICT,
      'Sản phẩm OCOP đã được người khác cập nhật, vui lòng tải lại',
      HttpStatus.CONFLICT,
      [{ latest: this.toAdminDto(latest) }],
    );
  }

  private notFound(): AppException {
    return new AppException(
      ErrorCode.OCOP_PRODUCT_NOT_FOUND,
      'Không tìm thấy sản phẩm OCOP',
      HttpStatus.NOT_FOUND,
    );
  }

  private slugExists(): AppException {
    return new AppException(
      ErrorCode.OCOP_PRODUCT_SLUG_EXISTS,
      'Slug sản phẩm OCOP đã tồn tại',
      HttpStatus.CONFLICT,
    );
  }
}
